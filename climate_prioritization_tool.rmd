...
#### Crop Calendar
This section is in development, you may see an error message.
```{r Dataset - Crop Calendar,echo=F}
crop_cal<-reactive({
  if(grepl("seasonal",input$timeframe_choice)){
    crop_cal<-terra::rast(paste0(country_dir(),"/crop_cal.tif"))
    crop_cal<-terra::mask(terra::crop(crop_cal,Geographies()$admin0),Geographies()$admin0)
    crop_cal
  }
})

renderPlot({terra::plot(crop_cal())},height=Plot.Height, width=Plot.Width)
...
renderPlot({terra::plot(cc_hist())},height=Plot.Height, width=Plot.Width)
```
...
4) Paletting issue for combinations - when haz combinations do not match between historic and future scenarios this causes issues. Can be replicated by selecting muliple hazards and reducing scale from country to smaller admin units.
